...
lidar::data::Point2D::Point2D(float angle, uint16_t distance, uint8_t intensity, uint64_t stamp)
     : angle(angle), distance(distance), intensity(intensity), time_stamp(stamp),
     x(distance * cos(angle)), y(distance * sin(angle))
{
}

lidar::data::Point2D::Point2D(double x, double y, float angle, uint16_t distance, uint8_t intensity, uint64_t stamp)
