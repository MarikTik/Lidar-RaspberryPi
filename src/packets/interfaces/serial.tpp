...
namespace lidar::transmission{
      ...
      std::vector<Packet> SerialInterface<Packet>::read(std::size_t amount) {
         ...
         static auto read_till_success = [&](int8_t* destination, unsigned size){ 
            int bytes_read = serRead(_handle, destination, size);
            while(bytes_read != size){
               std::cerr << "Error: Serial Interface recieved " << bytes_read << " bytes. Expected " << size;
               bytes_read = serRead(_handle, destination, size)
            }
         };
